...
int main(int argc, char **argv)
{
	...
	else if (openutils::sstring::to_sstring(argv[1]) == "--init")
	{
		...
		while (true)
		{
			...
			openutils::vector_t<openutils::sstring> tokens = command.append_start(argv[0]).to_argv();
	...
	else if (openutils::sstring::to_sstring(argv[1]) == "--open")
	{
		...
		for (std::size_t i = 0; i < spt.length(); i++)
		{
			...
			openutils::vector_t<openutils::sstring> vec = spt[i].append_start(argv[0]).to_argv();
